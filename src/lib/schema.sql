...
create policy "Allow users to update their own profile" on public.profiles
  for update using (auth.uid() = id OR (select current_setting('request.jwt.claims', true)::json->>'sub') = id);

create policy "Allow users to insert their own profile" on public.profiles
  for insert with check (auth.uid() = id OR (select current_setting('request.jwt.claims', true)::json->>'sub') = id);
